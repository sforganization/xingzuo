...
INTERRUPT_HANDLER(UART2_RX_IRQHandler, 21)
{
	...
	*/
	...
	if (SysInfor.u8RecPtr >= RECIVE_MAX)
		SysInfor.u8RecPtr = 0;

	SysInfor.u8Recive[SysInfor.u8RecPtr++] = Res;


	if ((SysInfor.u8RecPtr == 1) && (Res != 0xAA)) //包头
	{
		SysInfor.u8RecPtr	= 0;
	}
	else if (SysInfor.u8RecPtr == RECIVE_MAX) {
		if (Res == 0x55) //包尾
			SysInfor.bRecComplite = TRUE;
		else 
			SysInfor.u8RecPtr = 0;
	}
}
